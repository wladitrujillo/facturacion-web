...
export class CustomerService {
  ...
  delete(_id: string): Observable<any> {
    return this.http.delete<any>(`/api/customer/${_id}`).catch(this.handleError<any>('delete'));
  }

  get(filter: String, sort: string, page: number, sizePage: number): Observable<Customer[]> {
    return this.http.get<Customer[]>(`/api/customer?q=${filter}&sort=${sort}&page=${page}&per_page=${sizePage}`).catch(this.handleError<Customer[]>('get'));
  }

  getById(_id: string): Observable<Customer> {
    return this.http.get<Customer>(`/api/customer/${_id}`).catch(this.handleError<any>('getById'));
  }

  findByTaxId(taxId: string) {
    return this.http.get<Customer>(`/api/customer/taxId/${taxId}`).catch(this.handleError<any>('findByTaxId'));
  }
