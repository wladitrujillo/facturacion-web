...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  public ngOnInit() {
    ...
    this.dailySalesChart = new Chartist.Line('#dailySalesChart', dataDailySalesChart, optionsDailySalesChart);
  ...
  loadDailyData() {
    ...
    this.indicatorService.dailyIndicator(this.today.getFullYear(), this.today.getMonth() + 1, this.today.getDate())
      .subscribe(data => {
        let labels = data.map(item => item.description);
        let totals = data.map(item => item.total);
        dataDailySalesChart.labels = labels;
        dataDailySalesChart.series = [totals];
        let optionsDailySalesChart = {
          lineSmooth: Chartist.Interpolation.cardinal({
            tension: 0
          }),
          low: 0,
          high: Math.max(...totals) + 500, // creative tim: we recommend you to set the high sa the biggest value + something for a better look
          chartPadding: { top: 0, right: 0, bottom: 0, left: 0 },
        };
        this.dailySalesChart = new Chartist.Line('#dailySalesChart', dataDailySalesChart, optionsDailySalesChart);
        this.startAnimationForLineChart(this.dailySalesChart);
  ...
  loadMonthlyData() {
    ...
    this.indicatorService.monthlyIndicator(this.today.getFullYear())
      ...
      .subscribe(data => {
        ...
        dataWebsiteViewsChart.series = [totals];
